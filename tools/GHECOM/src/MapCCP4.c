...
>>>>> MRC/CCP4 2014 file format <<<<
  ...
  taken from "http://www.ccpem.ac.uk/mrc_format/mrc2014.php"
...
/*** FUNCTIONS (GLOBAL) ***/
int  Read_MapCCP4_File_VOXEL();
int  Write_MapCCP4_File_VOXEL();
...
static unsigned short Read_Unsigned_Short();
...
int Read_MapCCP4_File_VOXEL(vox,ifname,stdout_type,maxMbyte, readOnlyHeaders, readOnlyMinMax,sliceZnum, min_dens, max_dens)
  struct VOXEL *vox;
  char   *ifname; 
  ...
  char   readOnlyHeaders;  /* 'H':read only headers. */
  char   readOnlyMinMax;   /* 'M':read only min_density and max_density. */
  int    sliceZnum;        /* if (sliceZnum>=0) only read [sliceZnum]-th slice. */
  float  *min_dens, *max_dens;  /* minDensity and maxDensity, calculated only if readOnlyMinMax = 'M' */
{
 FILE *fp;
 char Order,buff[128];
 int L,i,j,x,y,z,malloc_ok,origNz;
 ...
 float SKWMAT[3][3],SKWTRN[3],ORIGIN[3], density;
 ...
 if (stdout_type!='F') {
   printf("#Read_MapCCP4_File_VOXEL(\"%s\" FileType '%c' maxMbyte %lf readOnlyHeaders %c readOnlyMinMax %c slizeZnum %d)\n",
       ifname,FileType,maxMbyte,readOnlyHeaders,readOnlyMinMax,sliceZnum);
 }
 ...
 if ((NC > 10000)||(NC<0)){  /* If NC is too large or negative, try Inverse byte order */
   ...
   printf("#NC %d\n",NC); 
   if ((NC > 10000)||(NC<0)){ 
     printf("#ERROR in density map (%s). NC=%d is not proper\n",ifname,NC);
   }
  }
 ...
 if ((Alpha !=90.0) || (Beta !=90.0) || (Gamma !=90.0)){
   printf("#WARNING: density map(%s) has non-orthogonal axis angles (Alpha %f Beta %f Gamma %f)\n",ifname,Alpha,Beta,Gamma); 
 }
 ...
 /** MRC **/
    ...
    printf("#IMPROPER FILETYPE '%c' for Read_MapCCP4_File().\n",FileType);
    exit(1);
  }
  ...
  if (readOnlyMinMax != 'M'){
    if (sliceZnum>=0){
      if (sliceZnum >= vox->N[2]){
        printf("#ERROR:sliceZnum(%d) is larger than vox->N[2](%d).\n",sliceZnum, vox->N[2]);
        exit(1);
      }
      vox->N[2] = 1;
      malloc_ok = Malloc_VOXEL(vox,vox->N[0],vox->N[1],1); 
    }
    else{
  ...
  */
  ...
  if (stdout_type != 'F'){
   ...
   printf("#N %d %d %d sliceZnum %d\n",vox->N[0],vox->N[1], vox->N[2], sliceZnum); 
  ...
  /** Read Voxel **/
   if (sliceZnum>=0){
     for (z=0;z<origNz;++z){
         for (y=0;y<vox->N[1];++y){
           for (x=0;x<vox->N[0];++x){ 
                   if (MODE==0){ density =  (float)Read_Char(fp,Order,"-");}
              else if (MODE==1){ density =  (float)Read_Short(fp,Order,"-");}
              else if (MODE==2){ density =  Read_Float(fp,Order,"-");}
              else if (MODE==6){ density =  (float)Read_Unsigned_Short(fp,Order,"-"); }
              if (z==sliceZnum){
                /* printf("#density %f\n",density); */
                vox->dat[x][y][0] = density;
                /* printf("#xyz %d %d %d %f\n",x,y,z,density); */
              }
           }
         }
       ...
       for (y=0;y<vox->N[1];++y){
         for (x=0;x<vox->N[0];++x){ 
            ...
            if (readOnlyMinMax == 'M'){
              ...
              if (density < *min_dens) {*min_dens = density;}
              if (density > *max_dens) {*max_dens = density;}
            }
            else{
                vox->dat[x][y][z] = density;
...
} /* end of Read_MapCCP4_File_VOXEL() */




int Write_MapCCP4_File_VOXEL(vox,fname)
  struct VOXEL *vox;
  char   *fname;
{
 FILE *fp;
 ...
 /*  
 vox->OrigPos[0] = NCSTART * vox->grid_width;
 vox->OrigPos[1] = NRSTART * vox->grid_width;
 vox->OrigPos[2] = NSSTART * vox->grid_width;
 */
 ...
 */
 ...
 sprintf(LABEL[0],"THIS IS MADE BY gm2Dimg");
 ...
 return(1);
} /* end of Write_MapCCP4_File_VOXEL() */
...
unsigned short Read_Unsigned_Short(fp,Order,comment)
 FILE *fp;
 char Order; /* 'I'nverse */
 char *comment;
/* short :: 2 bytes(16bits) */
{ unsigned short i;
  ...
  ret = fread(&i,1,2,fp); 
  if (ret<2) {printf("#ERROR Read_Unsigned_Short(%s):Can't read 2 chars.\n",comment); exit(1);}
  if (Order=='I'){Inverse_2bytes(&i);} 
 /* if (comment[0]!='-') printf("#%s %d\n",comment,i); */
  /* printf("unsigned short %d\n",i);  */
  return(i); 
} /* end of Read_Unsigned_Short() */
