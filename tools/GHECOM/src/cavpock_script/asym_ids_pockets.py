...
if (len(sys.argv)<2):
  print "asym_ids_pockets.py <options>"
...
olog.write("# -ilist  : input list file with (pdb_id) (asym_id) [%s]"%(OPT['ilist']))
...
lines = read_pdb_asym_list_file(OPT['ilist'])
...
for i in range(Nstart,Nend):
  pdb_asym = lines[i]
  (pdb_id, asym_id) = lines[i].split('_')
  ...
  command = "%s -M %s -icif %s -asym_id %s -gw %s -clus T -rl %f -rs %f -opocpdb %s"%(OPT['ghecom'],OPT['M'],iciffile,asym_id,OPT['gw'],rl,rs,opocpdbfile)
